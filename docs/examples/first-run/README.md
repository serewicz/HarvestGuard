...
| Generating version | `harvestguard 0.1.0` (what `harvestguard --version` prints, and what the report's *Scan Information* table records) |
| Repository state | commit `6c7ebce` plus this change, which adds only these examples and their tests — no scanner, report-generator, schema, or evidence-store behavior was changed to produce them |
