...
| Field | Value |
| --- | --- |
| Scan ID | synthetic\-unsupported\-001 |
...
| Evidence digest | 807b0b018a1703cd1f87bb039ed19d2dfec7f8b1e7251cc21231ea1fcae298ce |
| Retained snapshot occurrences | 1 |

**Evidence evaluation: INCOMPLETE**

> Evidence evaluation: INCOMPLETE. At least one required check or requested evaluation could not be completed or established for the declared scope. Retained evidence remains valid for what it records; what could not be established is listed as an unknown.

Why this evaluation:

- EV\-RSN\-002: 3 required check\(s\) could not be completed or established: EV\-CHK\-002 supported\_evidence\_schema \(unknown\), EV\-CHK\-003 supported\_collection\_contract \(unknown\), EV\-CHK\-004 execution\_completeness \(unknown\).
...
No defined exception was raised for this run. The closed set of defined exceptions is listed under [Defined exceptions](#defined-exceptions).
...
- Statement: Scanner\(s\) recorded for this run: semgrep\_crypto\_rules 0.1.0.
...
| Scanner versions | semgrep\_crypto\_rules 0.1.0 |
| Excluded paths | none |
...
| EV\-CHK\-002 | supported\_evidence\_schema | yes | yes | unknown |
| EV\-CHK\-003 | supported\_collection\_contract | yes | yes | unknown |
| EV\-CHK\-004 | execution\_completeness | yes | yes | unknown |
...
- State: passed
- Result: The stored run's recomputed digest matched the stored digest 807b0b018a1703cd1f87bb039ed19d2dfec7f8b1e7251cc21231ea1fcae298ce.
...
- State: unknown
- Result: Unsupported normalized\-finding schema version\(s\) 0.0.9 are present; executive policy 0.1.0 cannot establish that this evidence means what it appears to mean. The stored records are retained unchanged.
...
- Limitation: An unsupported schema version is not a defect in the stored evidence; it means this policy version does not define how to interpret it.
- Evidence: [synthetic\-unsupported\-001 ordinal 0 (finding ID 2adf7d326cca9a250b6785c912057fa2c6ac9421caf7ff54d791e9b3e137086f)](#evidence-synthetic-unsupported-001-0)

### EV\-CHK\-003 Supported collection contract

- State: unknown
- Result: No documented collection contract for scanner/version pair\(s\) semgrep\_crypto\_rules 0.1.0. Their execution\-failure semantics are unknown to executive policy 0.1.0.
...
- Source: semgrep\_crypto\_rules 0.1.0

### EV\-CHK\-004 Execution completeness

- State: unknown
- Result: The run recorded no scanner errors, but at least one scanner/version pair has no documented collection contract \(EV\-CHK\-003\), so an empty error list does not establish that every scanner ran to completion.
...
- State: passed
- Result: All 1 evidence reference\(s\) resolve to exactly one stored snapshot occurrence of run synthetic\-unsupported\-001.
...
No defined exception was raised for this run.

## Supported conclusions and limits

### EV\-CON\-001 (check\_result)

- Statement: The stored run's recomputed digest matched the stored digest 807b0b018a1703cd1f87bb039ed19d2dfec7f8b1e7251cc21231ea1fcae298ce.
- Method: evidence\_store.load\_scan\_run\(\) recomputed the stored run's SHA\-256 digest over its canonical run payload and ordered finding snapshots and compared it with the stored digest. verify\_loaded\_scan\_run\(\) rechecked the current payload with the same canonical digest definition and checked reconstruction consistency before this projection was built. A mismatch fails closed with EvidenceIntegrityError; no view is produced.
...
- Statement: The run recorded no scanner errors, but at least one scanner/version pair has no documented collection contract \(EV\-CHK\-003\), so an empty error list does not establish that every scanner ran to completion.
- Method: Read the run's recorded scanner\_errors, and required every recorded scanner/version pair to have a documented collection contract \(EV\-CHK\-003\) before an empty error list could be read as successful execution.
...
- EV\-CHK\-002 supported\_evidence\_schema: Unsupported normalized\-finding schema version\(s\) 0.0.9 are present; executive policy 0.1.0 cannot establish that this evidence means what it appears to mean. The stored records are retained unchanged.
- EV\-CHK\-003 supported\_collection\_contract: No documented collection contract for scanner/version pair\(s\) semgrep\_crypto\_rules 0.1.0. Their execution\-failure semantics are unknown to executive policy 0.1.0.
- EV\-CHK\-004 execution\_completeness: The run recorded no scanner errors, but at least one scanner/version pair has no documented collection contract \(EV\-CHK\-003\), so an empty error list does not establish that every scanner ran to completion.
...
<a id="evidence-synthetic-unsupported-001-0"></a>

### Occurrence 0

- Complete reference: scan synthetic\-unsupported\-001, ordinal 0, finding ID 2adf7d326cca9a250b6785c912057fa2c6ac9421caf7ff54d791e9b3e137086f

| Stored field | Value |
| --- | --- |
| finding\_id | 2adf7d326cca9a250b6785c912057fa2c6ac9421caf7ff54d791e9b3e137086f |
| scan\_id | synthetic\-unsupported\-001 |
...
| provenance.scanner\_version | 0.1.0 |
...
| schema\_version | 0.0.9 |

- Unrecognized stored field name(s): none. Every stored field of this snapshot is recognized by the exporting version.
